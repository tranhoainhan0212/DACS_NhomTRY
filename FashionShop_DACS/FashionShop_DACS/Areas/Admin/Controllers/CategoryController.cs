    ...
    public class CategoryController : Controller
    {
        ...
        public async Task<IActionResult> Add(Category category)
        {
            if (ModelState.IsValid)
            {
                ...
                await _httpClientService.PostDataAsync("http://localhost:7091/api/Category", content, Request.Cookies["token"]);
        ...
        public async Task<IActionResult> Update(int id, Category category)
        {
            ...
            {
                var content = new StringContent(JsonConvert.SerializeObject(category), Encoding.UTF8, "application/json");
                await _httpClientService.PutDataAsync(baseUrlCategory, content, Request.Cookies["token"]);
                return RedirectToAction(nameof(Index));
            }
            return View(category);
        }
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            if (category != null)
            {
                await _httpClientService.DeleteDataAsync($"{baseUrlCategory}?id={id}", Request.Cookies["token"]);
            }
